...
2. 读取 `site/site.json` 和各工作区自己的发布配置，作为发布站的可调配置
3. 根据配置生成聚合首页；若未手动配置版本更新文案，再回退读取各工作区最近的 Git 提交作为更新记录
4. 保留原始 demo 相对路径，避免图片和脚本在线上失效
5. 为复制过去的 Markdown 生成对应的 `.html` 浏览页，保证 demo 中的 README 链接在线可打开
6. 自动跳过超过 Cloudflare Pages 单文件上限的附件，避免大文件阻断部署

### 发布站文案怎么自己改

现在推荐按“全局一份、每个工作区各自一份”的方式改：

1. 全局站点配置：`site/site.json`
2. 工作区卡片配置：`workspaces/<workspace>/site.json`
3. 工作区更新记录：`workspaces/<workspace>/updates.md`

其中：

1. `site/site.json` 里放站点主标题、开头说明、首页分组，例如“主线版本 / 项目定制版本”
2. `workspaces/<workspace>/site.json` 里放该工作区的标题、说明、状态、按钮
3. `workspaces/<workspace>/updates.md` 里放该工作区的版本更新

工作区按钮推荐直接写相对当前工作区的路径，例如：

1. `demo/client.html`
2. `demo/platform.html`
3. `docs/方案说明.md`

外部链接仍然直接写完整网址，例如飞书链接。

版本更新推荐直接写在 `updates.md` 里，格式如下：

```md
- 2026-04-10｜发布站首页切换为聚合展示模式。
- 2026-04-09｜尺寸测量场景继续推进，重点联动尺寸工具、客户端和平台端。
```

首页默认只显示前 3 条，更多内容会折叠到“展开更多更新”里。
