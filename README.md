...
2. 读取 `site/demo-site.json`，作为发布站的可调配置
...
你后续如果想自己调整首页和各工作区文案，直接改这个文件：

1. `site/demo-site.json`

这里可以改：

1. 站点主标题
2. 站点开头说明
3. 首页分组，例如“主线版本 / 项目定制版本”
4. 每个工作区的标题
5. 每个工作区的说明
6. 每个工作区当前状态，例如“设计中 / 开发中 / 存档”
7. 每个工作区里要显示哪些按钮
8. 内部文档链接或外部链接，例如飞书文档
9. 每个工作区的版本更新日志文案

版本更新推荐直接写成这种最短格式：

1. `date`：日期，例如 `2026-04-10`
2. `text`：一句要展示的话，例如 `尺寸测量场景继续推进，重点联动尺寸工具、客户端和平台端。`
