...
const PLATFORM_LIBRARY_STORAGE_KEY = "jetcheck-platform-replica-v2";
...
function renderToolRuntime() {
  ...
  els.stopToolRunBtn.textContent = captureMode ? "结束采图" : "退出工具";
...
function renderRecordTable() {
  ...
  els.recordTableBody.innerHTML = pageRows
    .map((record) => {
      const checked = ui.selectedRecordIds.has(record.id) ? "checked" : "";
      const resultMarkup = renderBusinessBadge(record.businessResult || record.totalResult || "-");
      return `
        <tr>
          <td class="table-select-col"><input type="checkbox" data-action="select-record" data-id="${record.id}" aria-label="选择检测记录 ${escapeAttribute(record.id)}" ${checked} /></td>
          ...
          <td>
            <div class="camera-row-actions">
              <button class="table-btn table-btn-primary" data-action="view-record-detail" data-id="${record.id}">查看详情</button>
              <button class="table-btn table-btn-danger" data-action="delete-record" data-id="${record.id}">删除</button>
            </div>
          </td>
        </tr>
      `;
...
function deleteLocalModel(modelId) {
  ...
  const referenceNotice = referencedTools.length
    ? `
      <p>当前模型被以下工具引用。删除后，这些工具的相关配置会被清空。</p>
      <ul class="reference-confirm-list">
        ${referencedTools.map((name) => `<li>${escapeHtml(name)}</li>`).join("")}
      </ul>
    `
    : `<p>确定删除模型“${escapeHtml(model.modelName)} / ${escapeHtml(model.version)}”吗？</p>`;
...
function confirmDeleteRecords(recordIds) {
  ...
  openSharedModal({
    ...
    onConfirm() {
      ...
      state.detectionRecords = state.detectionRecords.filter((record) => !deleteIds.has(record.id));
...
function exportCaptureImages(imageIds, batch = getActiveCaptureBatch()) {
  ...
  showToast(`已触发浏览器下载：${batch.id}_${images.length}张图像.zip`);
}
...
function applyCaptureImageTag(batch, imageIds, tag) {
  ...
  saveStateSilently();
  showToast(`已为 ${ids.size} 张图像${shouldRemove ? "移除" : "添加"}标签：${tag}`);
}
...
function openCaptureCloudSyncModal(toolId, imageIds) {
  ...
  openSharedModal({
    ...
    onConfirm() {
      ...
      closeSharedModal();
      showToast(`已同步 ${images.length} 张图像到云端图像库`);
      openCaptureToolLibrary(toolId);
      return true;
    },
  });
...
function openCaptureToolLibrary(toolId, options = {}) {
  ...
  openSharedModal({
    ...
    onConfirm() {
      ...
      showToast(`已触发浏览器下载：${tool.name}_${count}张图像.zip`);
      return false;
    },
  });
}
...
function openCaptureBatchExportModal(batchIds) {
  ...
  openSharedModal({
    ...
    onConfirm() {
      ...
      closeSharedModal();
      showToast(`已触发浏览器下载：采图记录导出_${formatExportFileTimestamp(state.meta.now)}.zip`);
      return true;
    },
  });
}
...
function openRecordExportModal(recordIds = []) {
  ...
  const rows = state.detectionRecords.filter((record) => selectedIds.has(record.id));
  ...
  if (captureRows.length && rows.length) {
    ...
    openSharedModal({
      ...
      onConfirm() {
        closeSharedModal();
        showToast(`已触发浏览器下载：记录批量导出_${formatExportFileTimestamp(state.meta.now)}.zip`);
        return true;
      },
    });
    return;
...
function getFilteredRecords() {
  return getAllRecordRows().filter((record) => {
    ...
    const businessMatch =
      ui.recordFilters.business === "all" ||
      (record.kind === "capture"
        ? getCaptureRecordTags(record).includes(ui.recordFilters.business)
        : (record.businessResult || record.totalResult) === ui.recordFilters.business);
    const keyword = ui.recordFilters.keyword.toLowerCase();
    const tagKeyword = (record.kind === "capture" ? getCaptureRecordTags(record) : getRuntimeRecordTags(record)).join(" ").toLowerCase();
    const keywordMatch =
      !keyword ||
    ...
    return runModeMatch && businessMatch && keywordMatch && startMatch && endMatch;
  });
}

function getAllRecordRows() {
  return state.detectionRecords
    .map((record) => ({ ...record, kind: "detection" }))
  ...
  return `
    <article class="builder-item">
      <div class="builder-item-main">
        <div class="builder-item-title">
          <strong>${escapeHtml(item.name)}</strong>
          <span class="chip">${escapeHtml(sceneType === "尺寸" ? "尺寸方案" : "本地模型")}</span>
        </div>
        <div class="builder-meta">
...
function getParamFieldSchema() {
  return [
    ...
    { key: "gain", label: "增益 (dB):", type: "number", defaultValue: 0 },
  ];
}
