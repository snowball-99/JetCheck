...
const DIMENSION_ELEMENT_ITEMS = [
  {
    id: "element_line_1",
    name: "直线段1",
    category: "元素",
    typeLabel: "直线段",
    toolKey: "line_segment",
    status: "ok",
    statusLabel: "有效",
    source: "手动绘制",
    coordinate: "图像内",
  ...
  {
    id: "construct_intersection_1",
    name: "线线交点1",
    category: "构造",
    typeLabel: "点",
    toolKey: "line_intersection",
    status: "error",
    statusLabel: "异常",
    source: "直线段1 + 直线段2",
    coordinate: "X 142.6%，Y -18.4%",
    relation: "近似平行线求交",
    detail: "交点超出图像边界，无法在画布中直接点击。请在元素列表中定位或删除。",
  },
  {
    id: "measure_distance_1",
    name: "螺杆1总长",
    category: "测量",
    typeLabel: "线线距离",
    toolKey: "line_distance",
    status: "ok",
    statusLabel: "有效",
    source: "直线段1 + 直线段2",
    coordinate: "输出 16.443 mm",
    relation: "尺寸结果元素",
    detail: "测量项作为元素参与统一管理，可继续被检测规则和发布结果引用。",
  },
];
...
const state = {
  ...
  selectedMeasureElementId: "construct_intersection_1",
  hiddenMeasureElementIds: [],
...
function getDefaultRoiNodes() {
  return [
    { type: "roiRange", x: 0, y: 0, w: 100, h: 100 },
    { type: "roiTarget", x: 0, y: 0, w: 100, h: 100 },
  ];
}
...
function getVisibleDimensionElements() {
  const hiddenIds = new Set(state.hiddenMeasureElementIds || []);
  return DIMENSION_ELEMENT_ITEMS.filter((item) => !hiddenIds.has(item.id));
}

function getSelectedDimensionElement() {
  const elements = getVisibleDimensionElements();
  return elements.find((item) => item.id === state.selectedMeasureElementId) || null;
}

function getDimensionElementInfoFields(element) {
  return [
    { label: "元素名称", value: element.name, hint: "用于在后续构造和测量中引用该元素。" },
    { label: "元素类型", value: `${element.category} / ${element.typeLabel}`, hint: "元素来源分类和输出类型。" },
    { label: "构造状态", value: element.statusLabel, hint: "元素当前是否可用于后续测量。" },
    { label: "输入元素", value: element.source, hint: "构造该元素所依赖的上游元素。" },
    { label: "位置", value: element.coordinate, hint: "元素在模板图坐标系中的位置。" },
    { label: "说明", value: element.detail, hint: "异常或使用说明。" },
  ];
}

function renderElementListPanel() {
  const elements = getVisibleDimensionElements();
  return `
    <section class="dt-workbench-panel dt-element-list-panel">
      <div class="dt-workbench-panel-head">
        <strong>元素列表</strong>
      </div>
      <div class="dt-element-list">
        ${elements.length
          ? elements
              .map((item) => {
                const active = item.id === state.selectedMeasureElementId ? "is-active" : "";
                return `
                  <article class="dt-element-row ${active} is-${item.status}" data-dimension-element-id="${item.id}">
                    <button type="button" class="dt-element-row-main" data-element-action="select" data-element-id="${item.id}">
                      <span class="dt-element-status-dot"></span>
                      <span class="dt-element-row-copy">
                        <strong>${item.name}</strong>
                        <em>${item.category} / ${item.typeLabel}</em>
                      </span>
                      <b>${item.statusLabel}</b>
                    </button>
                    <div class="dt-element-row-actions">
                      <button type="button" data-element-action="locate" data-element-id="${item.id}">定位</button>
                      <button type="button" class="is-danger" data-element-action="delete" data-element-id="${item.id}">删除</button>
                    </div>
                  </article>
                `;
              })
              .join("")
          : `<div class="dt-empty-mini">暂无元素</div>`}
      </div>
    </section>
  `;
}
...
function renderElementInfoPanel() {
  const selectedElement = getSelectedDimensionElement();
  if (selectedElement) {
    return `
      <section class="dt-workbench-panel dt-config-panel">
        ...
        <div class="dt-panel-scroll">
          <div class="dt-element-alert is-${selectedElement.status}">
            <strong>${selectedElement.statusLabel}</strong>
            <span>${selectedElement.detail}</span>
          </div>
          ${getDimensionElementInfoFields(selectedElement)
            .map((field) => renderInfoInput(field.label, field.value, field.hint, field.type || "text"))
            .join("")}
        </div>
      </section>
    `;
...
function renderStepScaffold(project, tab) {
  ...
  const releaseState = getReleaseReadiness(project);
  const showElementSidebar = isMeasureConfig || (isCalibration && state.calibrationMode === "manual" && calibrationImageReady);
  ...
  refs.workspace.classList.toggle("has-element-sidebar", showElementSidebar);
  refs.leftPanel.hidden = !showElementSidebar;
  ...
  if (showElementSidebar) {
    refs.leftPanel.innerHTML = `
      ${renderToolboxPanel({ toolsDisabled: isMeasureConfig && !templateReady })}
      ${renderElementListPanel()}
    `;
  }
...
if (refs.leftPanel) {
  refs.leftPanel.addEventListener("click", (event) => {
    ...
    const elementActionButton = event.target.closest("[data-element-action]");
    if (elementActionButton) {
      const elementId = elementActionButton.dataset.elementId || "";
      const element = DIMENSION_ELEMENT_ITEMS.find((item) => item.id === elementId);
      if (!element) return;
      state.selectedMeasureElementId = elementId;
      state.measureElementSelected = true;
      state.measureToolKey = element.toolKey || state.measureToolKey;
      const matchedSection = MEASURE_TOOL_SECTIONS.find((section) =>
        section.tools.some((tool) => tool.key === state.measureToolKey)
      );
      if (matchedSection) state.measureToolSectionKey = matchedSection.key;

      if (elementActionButton.dataset.elementAction === "delete") {
        state.hiddenMeasureElementIds = Array.from(new Set([...(state.hiddenMeasureElementIds || []), elementId]));
        if (state.selectedMeasureElementId === elementId) state.selectedMeasureElementId = "";
        setManagerMessage(`已删除元素“${element.name}”`);
        render();
        return;
      }

      if (elementActionButton.dataset.elementAction === "locate") {
        setManagerMessage(
          element.status === "error"
            ? `“${element.name}”超出图像边界，已在元素列表中选中，可删除后重新构造。`
            : `已定位到“${element.name}”。`
        );
        render();
        return;
    ...
    state.measureElementSelected = true;
    state.selectedMeasureElementId = "";
    const matchedSection = MEASURE_TOOL_SECTIONS.find((section) =>
      section.tools.some((tool) => tool.key === state.measureToolKey)
    );
    if (matchedSection) {
