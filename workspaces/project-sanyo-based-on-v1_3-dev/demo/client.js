...
const els = {
  ...
  runtimeBatchNoInput: document.getElementById("runtimeBatchNoInput"),
  runtimeSerialStartInput: document.getElementById("runtimeSerialStartInput"),
  runtimeNextWorkpieceCode: document.getElementById("runtimeNextWorkpieceCode"),
...
function bindStaticEvents() {
  ...
  els.runtimeBatchNoInput.addEventListener("input", renderRuntimeTraceabilityDraftPreview);
  els.runtimeSerialStartInput.addEventListener("input", renderRuntimeTraceabilityDraftPreview);
...
function renderToolRuntime() {
  ...
  ensureTraceabilityState();
  if (activeImageResult) ui.activeRuntimeImageId = activeImageResult.id;

  if (document.activeElement !== els.runtimeBatchNoInput) {
    els.runtimeBatchNoInput.value = state.traceability.batchNo || "";
  }
  if (document.activeElement !== els.runtimeSerialStartInput) {
    els.runtimeSerialStartInput.value = String(state.traceability.serialStart ?? DEFAULT_TRACEABILITY_STATE.serialStart);
  }
  renderRuntimeTraceabilityDraftPreview();
    ...
    .map((record) => {
      return `
        <tr>
          ...
          <td>${escapeHtml(record.workpieceCode || "-")}</td>
...
function startDetectionRun() {
  ...
  if (!applyRuntimeTraceabilityDraft()) {
    return;
  }
  Demo.saveState(state);
  renderRuntimeTraceabilityDraftPreview();
  ...
  timers.detection = window.setTimeout(() => {
    ...
    try {
      ...
      state.traceability.nextSerial = normalizeTraceabilitySerial((state.traceability.nextSerial ?? DEFAULT_TRACEABILITY_STATE.nextSerial) + 1, 9999);
...
function renderRecordDetailModal(record) {
  ...
  return `
    <div class="record-detail-runtime-layout" id="recordDetailModalBody">
      <div class="record-detail-summary-strip">
        <article class="record-detail-section record-detail-summary-item">
          <span>批次号</span>
          <strong>${escapeHtml(record.batchNo || "-")}</strong>
        </article>
        <article class="record-detail-section record-detail-summary-item">
          <span>工件编号</span>
          <strong>${escapeHtml(record.workpieceCode || "-")}</strong>
        </article>
        <article class="record-detail-section record-detail-summary-item">
          <span>触发时间</span>
          <strong>${escapeHtml(Demo.formatDateTime(record.triggeredAt))}</strong>
        </article>
        <article class="record-detail-section record-detail-summary-item">
          <span>业务结果</span>
          <div class="record-detail-summary-content">${renderBusinessBadge(record.businessResult || record.totalResult || "-") || "-"}</div>
        </article>
      </div>
...
function getFilteredRecords() {
  return state.detectionRecords.filter((record) => {
    ...
    const keywordMatch =
      ...
      String(record.workpieceCode || "").toLowerCase().includes(keyword) ||
...
function buildDetectionRecord(tool, runMode = "detect") {
  ...
  const serialNo = normalizeTraceabilitySerial(traceability.nextSerial, traceability.serialStart);
  const batchNo = String(traceability.batchNo || "").trim();
  const workpieceCode = buildWorkpieceCode(batchNo, serialNo);
...
function ensureTraceabilityState() {
  if (!state.traceability || typeof state.traceability !== "object") {
    state.traceability = { ...DEFAULT_TRACEABILITY_STATE };
  }
  state.traceability.batchNo = String(state.traceability.batchNo || "").trim();
  state.traceability.serialStart = normalizeTraceabilitySerial(state.traceability.serialStart, DEFAULT_TRACEABILITY_STATE.serialStart);
  state.traceability.nextSerial = normalizeTraceabilitySerial(state.traceability.nextSerial, state.traceability.serialStart);
  return state.traceability;
}

function normalizeTraceabilitySerial(value, fallback = DEFAULT_TRACEABILITY_STATE.serialStart) {
  const numeric = Number(value);
  if (!Number.isInteger(numeric)) return fallback;
  return Math.max(0, Math.min(9999, numeric));
}

function formatTraceabilitySerial(value) {
  return String(normalizeTraceabilitySerial(value, 0)).padStart(4, "0");
}

function buildWorkpieceCode(batchNo, serialNo) {
  const normalizedBatch = String(batchNo || "").trim();
  if (!normalizedBatch) return "-";
  return `${normalizedBatch}${formatTraceabilitySerial(serialNo)}`;
}

function getRuntimeTraceabilityDraft() {
  ensureTraceabilityState();
  return {
    batchNo: String(els.runtimeBatchNoInput?.value ?? state.traceability.batchNo ?? "").trim(),
    serialStartRaw: String(els.runtimeSerialStartInput?.value ?? state.traceability.serialStart ?? DEFAULT_TRACEABILITY_STATE.serialStart).trim(),
  };
}

function validateTraceabilityDraft(batchNo, serialStartRaw) {
  if (!batchNo) return "请输入批次号";
  const serialStart = Number(serialStartRaw);
  if (!Number.isInteger(serialStart) || serialStart < 0 || serialStart > 9999) {
    return "请输入 0-9999 的后缀起始位";
  }
  return "";
}

function applyRuntimeTraceabilityDraft(options = {}) {
  const { persist = false } = options;
  const draft = getRuntimeTraceabilityDraft();
  const errorMessage = validateTraceabilityDraft(draft.batchNo, draft.serialStartRaw);
  if (errorMessage) {
    showToast(errorMessage);
    return null;
  }
  const serialStart = Number(draft.serialStartRaw);
  const traceability = ensureTraceabilityState();
  const changed = draft.batchNo !== traceability.batchNo || serialStart !== traceability.serialStart;
  traceability.batchNo = draft.batchNo;
  traceability.serialStart = serialStart;
  traceability.nextSerial = changed ? serialStart : normalizeTraceabilitySerial(traceability.nextSerial, serialStart);
  if (persist) {
    persistState(`批次信息已更新：${traceability.batchNo} / 后缀从 ${formatTraceabilitySerial(traceability.serialStart)} 开始`);
  }
  return { ...traceability, changed };
}

function renderRuntimeTraceabilityDraftPreview() {
  ensureTraceabilityState();
  const draft = getRuntimeTraceabilityDraft();
  const serialError = validateTraceabilityDraft(draft.batchNo || "placeholder", draft.serialStartRaw);
  const serialStart = serialError ? state.traceability.serialStart : Number(draft.serialStartRaw);
  const draftChanged = draft.batchNo !== state.traceability.batchNo || serialStart !== state.traceability.serialStart;
  const previewSerial = draftChanged ? serialStart : state.traceability.nextSerial;
  els.runtimeNextWorkpieceCode.textContent = buildWorkpieceCode(draft.batchNo || state.traceability.batchNo, previewSerial);
}
