(function () {
  const STORAGE_KEY = "jetcheck-demo-state-sanyo-v1";
  ...
  function normalizeLoadedState(parsed) {
    ...
    next.traceability = next.traceability && typeof next.traceability === "object" ? next.traceability : {};
    next.traceability.batchNo = String(next.traceability.batchNo || "").trim();
    next.traceability.serialStart = Number.isInteger(Number(next.traceability.serialStart))
      ? Math.max(0, Math.min(9999, Number(next.traceability.serialStart)))
      : DEFAULT_STATE.traceability.serialStart;
    next.traceability.nextSerial = Number.isInteger(Number(next.traceability.nextSerial))
      ? Math.max(0, Math.min(9999, Number(next.traceability.nextSerial)))
      : next.traceability.serialStart;
    ...
    next.localModels = next.localModels.map((model) => ({
      ...
      categories: normalizeModelCategories(model),
    }));
    if (loadedVersion < 4) {
    ...
    next.detectionRecords = next.detectionRecords.map((record) => {
      ...
      return {
        ...
        serialNo: Number.isInteger(Number(record?.serialNo)) ? Number(record.serialNo) : null,
        workpieceCode: String(record?.workpieceCode || ""),
